    ...
    <div role="tablist" className={cn('inline-flex items-center gap-1 rounded-lg bg-slate-100 p-1 max-w-full overflow-x-auto scrollbar-thin', className)}>
      {options.map((o) => (
        <button
          ...
          className={cn('h-7 rounded-md px-2.5 text-[13px] font-medium whitespace-nowrap transition-colors flex items-center gap-1.5', value === o.value ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-600 hover:text-slate-900')}
        >
          {o.label}
