...
import { JOB_STATUS_LABELS, JOB_STATUS_SHORT_LABELS } from '../../domain/catalog';
import { cn } from '../cn';
 ...
 * cards — this is the triage view, not a replacement for it. Below `sm` the
 * table collapses to stacked rows, since five columns don't fit on a phone.
 */
  ...
  return (
    <div className="overflow-x-auto">
      {/* Fixed layout with explicit widths: left to size themselves, the job name and
          ...
          the card. Long values truncate instead, with the full text still on the link. */}
      <table className="w-full text-left border-collapse sm:table-fixed" data-testid="attention-table">
...
function AttentionRow({ item, view }: { item: AttentionItem; view: JobView }) {
  ...
  return (
    <tr className="block sm:table-row border-b border-slate-100 last:border-0 hover:bg-slate-50/60 transition-colors" data-testid="attention-row" data-severity={item.severity}>
      <td className="block sm:table-cell py-2.5 px-3 align-middle">
        <div className="flex items-center gap-2.5 min-w-0">
          <Link to={`/clients/${view.client.id}`} className="text-[13px] font-semibold text-slate-900 hover:text-primary-700 truncate" title={view.client.name}>
            {view.client.name}
          </Link>
      ...
      <td className="block sm:table-cell py-0.5 sm:py-2.5 px-3 align-middle">
        <Link to={`/jobs/${view.job.id}`} className="text-[13px] text-slate-700 hover:text-primary-700 truncate block" title={view.job.name}>
          {view.job.name}
        </Link>
      </td>
      <td className="block sm:table-cell py-0.5 sm:py-2.5 px-3 align-middle whitespace-nowrap">
      ...
      <td className="block sm:table-cell py-1 sm:py-2.5 px-3 align-middle">
      ...
      <td className="block sm:table-cell py-2 sm:py-2.5 px-3 align-middle">
      ...
      <td className="hidden sm:table-cell py-2.5 pr-2 align-middle">
